 ...
 * _cgiBinSuffix追加
 * 不正なパスを実行するバグを修正
 * {@link #canRun(HttpServletRequest, HttpServletResponse)}追加
 *
 * @see CGI
 */
public class CgiServlet extends HttpServlet
{
	...
	private static final Logger LOG = Log.getLogger(CgiServlet.class);
	...
	public void init() throws ServletException
	{
		...
		_cgiBinSuffix = getInitParameter("cgiBinSuffix");
	...
	public void service(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
	{
		...
		if (LOG.isDebugEnabled()) {
			...
			LOG.debug("CGI: _cgiBinSuffix: " + _cgiBinSuffix);
		}
		...
		if (!execCmd.getName().endsWith(_cgiBinSuffix)) {
			res.sendError(500);
			return;
		}
	...
	public boolean canRun(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
	{
		if (!_ok) {
			return false;
		}
	...
	private void exec(File command, String pathInfo, HttpServletRequest req, HttpServletResponse res) throws IOException
	{
		...
		if ((HttpMethod.POST.is(req.getMethod()) || HttpMethod.PUT.is(req.getMethod())) && "application/x-www-form-urlencoded".equals(req.getContentType())) {
			...
			bodyFormEncoded = UrlEncoded.encode(parameterMap, Charset.forName(req.getCharacterEncoding() == null ? "utf-8" : req.getCharacterEncoding()), true);
		}
